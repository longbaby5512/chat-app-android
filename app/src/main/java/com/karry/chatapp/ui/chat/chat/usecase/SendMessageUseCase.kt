...
class SendMessageUseCase @Inject constructor(
    ...
    operator fun invoke(message: MessageSent): Flow<Resource<Message>> = flow {
        ...
        cypher.init(Cipher.ENCRYPT_MODE, key.fromBase64Url())
        val encryptMessage = cypher.doFinal(content)
        Timber.d("Message sent after encrypt: ${encryptMessage.toBase64Url()}")
        val messageSent = message.copy(content = encryptMessage.toBase64Url())
