...
class SocketExecutor(private val client: Socket) {
    ...
    private suspend fun connect(method: Method, args: Array<Any>) {
        ...
        client.io().on(Manager.EVENT_TRANSPORT) {
        ...
        return suspendCoroutine {
            client.once(Socket.EVENT_CONNECT) { _ ->
                it.resume(Unit)
            }
        }
    }
    ...
    private suspend fun disconnect() {
        client.disconnect()
        return suspendCoroutine {
            client.once(Socket.EVENT_DISCONNECT) { _ ->
                it.resume(Unit)
            }
        }
    }
    ...
    private suspend fun emit(method: Method, args: Array<Any>): Any {
        ...
        return suspendCoroutine {
            client.emit(event, json, object : Ack {
